...
Release automation in the upstream Harness project follows the same distribution
contract: supported platforms receive prebuilt CLI assets, and target
repositories use the repository-local binary path.
...
- The `.agent-harness/bin/harness-cli` command is the stable entrypoint for
...
- Keep target repositories independent of a local Rust toolchain.
