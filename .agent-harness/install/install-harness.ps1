...
function Get-SourceMode {
    if ($PSScriptRoot) {
        ...
        $candidate = Split-Path -Parent $harnessDir
        if ((Test-Path (Join-Path $candidate "AGENTS.md")) -and (Test-Path (Join-Path $harnessDir "HARNESS.md"))) {
...
- `.agent-harness/bin/harness-cli query matrix` on macOS/Linux, or `.\.agent-harness\bin\harness-cli.ps1 query matrix` on Windows

Use the Rust Harness CLI at `.agent-harness/bin/harness-cli` on macOS/Linux or
`.agent-harness/bin/harness-cli.ps1` on Windows as the main operational tool.
<!-- HARNESS:END -->
'@
}
...
function Read-CliReleaseTag {
    $relative = ".agent-harness/install/harness-cli-release-tag"
    if ($script:Source.Mode -eq "local") {
...
function Install-HarnessCliBinary {
    ...
    $target = Join-Path $script:TargetDir ".agent-harness/bin/harness-cli.exe"

    if ((Test-Path $target) -and $script:ConflictAction -eq "merge" -and !$Force) {
        Write-Step "skip     .agent-harness/bin/harness-cli.exe (merge keeps existing file)"
        $script:Skipped++
        return
    }

    if ($DryRun) {
        Write-Step "download $binaryName -> .agent-harness/bin/harness-cli.exe"
    ...
    try {
        ...
        if (Test-Path $target) {
            if ($Force) {
                $backup = Join-Path $script:BackupDir ".agent-harness/bin/harness-cli.exe"
                New-Item -ItemType Directory -Force -Path (Split-Path -Parent $backup) | Out-Null
                Copy-Item -LiteralPath $target -Destination $backup -Force
            }
            $script:Updated++
            Write-Step "updated  .agent-harness/bin/harness-cli.exe"
        } else {
            $script:Created++
            Write-Step "created  .agent-harness/bin/harness-cli.exe"
        }
        Copy-Item -LiteralPath $binaryTmp -Destination $target -Force
        Write-Step "verified .agent-harness/bin/harness-cli.exe ($platform)"
    } finally {
...
$conflicts = @("AGENTS.md", ".agent-harness") | Where-Object { Test-Path (Join-Path $script:TargetDir $_) }
if ($conflicts.Count -gt 0) {
    if ($Merge) {
        ...
        foreach ($protected in @("AGENTS.md", ".agent-harness")) {
