...
Apply the agent-first Harness files and folders to a target project directory.
...
Options:
  ...
  -y, --yes              Accept defaults and skip prompts.
      ...
      --claude           Also install or refresh CLAUDE.md so Claude Code
                         ...
                         and .agent-harness/FEATURE_INTAKE.md inside a marked block.
      ...
      --override         On protected-path conflict, back up and replace
                         AGENTS.md and .agent-harness/.
      --force            Overwrite existing files after backing them up.
      --dry-run          Show what would change without writing files.
  -h, --help             Show this help.

Safety:
  If AGENTS.md or .agent-harness/ already exists, interactive installs
  ask whether to merge missing files, override after backup, or stop. Merge is
  the safe update path for repositories that already have Harness: existing
  files stay in place and new Harness files are appended by path. Non-
...
Examples:
  .agent-harness/install/install-harness.sh
  .agent-harness/install/install-harness.sh --directory /path/to/project --yes
  .agent-harness/install/install-harness.sh ./my-project --force
  curl -fsSL https://raw.githubusercontent.com/hoangnb24/repository-harness/main/.agent-harness/install/install-harness.sh | bash -s -- --yes
  curl -fsSL https://raw.githubusercontent.com/hoangnb24/repository-harness/main/.agent-harness/install/install-harness.sh | bash -s -- --merge --yes
  curl -fsSL https://raw.githubusercontent.com/hoangnb24/repository-harness/main/.agent-harness/install/install-harness.sh | bash -s -- --merge --refresh-agent-shim --yes
  curl -fsSL https://raw.githubusercontent.com/hoangnb24/repository-harness/main/.agent-harness/install/install-harness.sh | bash -s -- --claude --yes
EOF
}
...
if grep -Fxq "harness.db" "$target" &&
   ...
   grep -Fxq ".agent-harness/bin/harness-cli.bin" "$target" &&
   grep -Fxq ".agent-harness/bin/harness-cli.exe" "$target"; then
...
- `.agent-harness/FEATURE_INTAKE.md`
- `.agent-harness/ARCHITECTURE.md`
- `.agent-harness/CONTEXT_RULES.md`
- `.agent-harness/TOOL_REGISTRY.md`
- `.agent-harness/bin/harness-cli query matrix`

Use the Rust Harness CLI at `.agent-harness/bin/harness-cli` as the main operational
tool. Before a step that could use an external tool, run
`.agent-harness/bin/harness-cli query tools --capability <name> --status present` to
...
context (the "Must in all lanes" set from `.agent-harness/CONTEXT_RULES.md`) at
...
Also run `.agent-harness/bin/harness-cli query matrix` before starting work.

Lane-dependent context (`README.md`, `.agent-harness/HARNESS.md`,
`.agent-harness/ARCHITECTURE.md`, `.agent-harness/CONTEXT_RULES.md`, product
docs, stories, decisions) is intentionally not imported — read it per lane, as
`.agent-harness/CONTEXT_RULES.md` prescribes.
<!-- HARNESS:END -->
EOF
}
...
read_cli_release_tag() {
  local tag_file=".agent-harness/install/harness-cli-release-tag"
  local tag=""
...
install_harness_cli_binary() {
  ...
  target="$TARGET_DIR/.agent-harness/bin/harness-cli.bin"

  if [ -e "$target" ] && [ "$CONFLICT_ACTION" = "merge" ] && [ "$FORCE" -eq 0 ]; then
    log "skip     .agent-harness/bin/harness-cli.bin (merge keeps existing file)"
    SKIPPED=$((SKIPPED + 1))
    return 0
  fi

  if [ "$DRY_RUN" -eq 1 ]; then
    log "download $binary_name -> .agent-harness/bin/harness-cli.bin"
  ...
  if [ -e "$target" ]; then
    if [ "$FORCE" -eq 1 ]; then
      mkdir -p "$BACKUP_DIR/.agent-harness/bin"
      cp -p "$target" "$BACKUP_DIR/.agent-harness/bin/harness-cli.bin"
    fi
    UPDATED=$((UPDATED + 1))
    log "updated  .agent-harness/bin/harness-cli.bin"
  else
    CREATED=$((CREATED + 1))
    log "created  .agent-harness/bin/harness-cli.bin"
  fi

  cp "$binary_tmp" "$target"
  chmod 755 "$target"
  rm -rf "$tmp_dir"
  log "verified .agent-harness/bin/harness-cli.bin ($platform)"
}
...
check_protected_target_paths() {
  ...
  [ -e "$TARGET_DIR/.agent-harness" ] && conflicts+=(".agent-harness/")
...

  {
    ...
    printf '  2. Override Back up and replace AGENTS.md and .agent-harness/\n'
...
override_protected_target_paths() {
  ...
  for protected in AGENTS.md .agent-harness; do
...
if [ -n "$SCRIPT_DIR" ] && [ -f "$SCRIPT_DIR/../../AGENTS.md" ] && [ -f "$SCRIPT_DIR/../HARNESS.md" ]; then
...
if [ "$YES" -eq 0 ] && can_prompt; then
  prompt_tty "Install agent-first Harness into [$TARGET_INPUT]: "
...
.agent-harness/TEST_MATRIX.md
.agent-harness/TOOL_REGISTRY.md
.agent-harness/TRACE_SPEC.md
.agent-harness/decisions/0001-harness-first-development.md
.agent-harness/decisions/0003-generic-spec-intake-harness.md
.agent-harness/decisions/0004-sqlite-durable-layer.md
.agent-harness/decisions/0005-prebuilt-rust-harness-cli.md
.agent-harness/decisions/0007-improvement-proposal-rules.md
.agent-harness/decisions/README.md
.agent-harness/product/README.md
.agent-harness/stories/README.md
.agent-harness/templates/decision.md
.agent-harness/templates/spec-intake.md
.agent-harness/templates/story.md
.agent-harness/templates/validation-report.md
.agent-harness/templates/high-risk-story/design.md
.agent-harness/templates/high-risk-story/execplan.md
.agent-harness/templates/high-risk-story/overview.md
.agent-harness/templates/high-risk-story/validation.md
.agent-harness/bin/harness-cli
.agent-harness/bin/harness-cli.ps1
.agent-harness/install/README.md
.agent-harness/install/harness-cli-release-tag
.agent-harness/install/install-harness.sh
.agent-harness/install/install-harness.ps1
.agent-harness/scripts/schema/001-init.sql
.agent-harness/scripts/schema/002-story-verify.sql
.agent-harness/scripts/schema/003-tool-registry.sql
.agent-harness/scripts/schema/004-intervention.sql
.agent-harness/scripts/schema/005-tool-extensions.sql
.gitignore
EOF
