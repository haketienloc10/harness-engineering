...
| --- | --- | --- | --- | --- | --- |
| 1 | Task specification | Covered | `AGENTS.md`, `_harness/FEATURE_INTAKE.md`, `_harness/templates/story.md`, `docs/stories/*`, task and intake records | Requests are classified by type and lane before implementation; normal and high-risk work use the progressive story template when a story is required. | Keep story packets synchronized with future product docs. |
| 2 | Context selection | Covered | `AGENTS.md`, `_harness/CONTEXT_RULES.md`, `_harness/ARCHITECTURE.md`, `docs/decisions/*`, task context acknowledgements | Phase-by-lane context rules and retrieval triggers are documented and measurable. | Future automation could enforce context selection instead of only measuring it. |
...
| 5 | Task state | Covered | `_harness/bin/harness-cli task`, `_harness/bin/harness-cli query matrix`, `_harness/TEST_MATRIX.md`, task, intake, proof, and trace records | The command-first lifecycle tracks intake, context, proof, trace, and terminal task state. | Fallback markdown can drift and should be kept small. |
| 6 | Observability | Partial | `_harness/TRACE_SPEC.md`, `trace` table, `_harness/bin/harness-cli task trace`, `query traces`, `query friction` | Task traces can be queried with friction context. | No dashboard or benchmark ingestion exists in this payload. |
...
| 8 | Verification | Covered | `_harness/TEST_MATRIX.md`, `proof run`, `proof query`, `query matrix`, `task trace`, `_harness/templates/story.md` | Repeatable proof is recorded against lifecycle tasks and summarized by the proof matrix and final trace. | Proof coverage remains task-specific. |
