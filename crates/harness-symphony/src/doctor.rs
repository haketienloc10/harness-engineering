...
fn check_database_or_changesets(config: &ResolvedConfig) -> DoctorCheck {
    ...
    if config.changeset_directory.exists() {
        return DoctorCheck {
            ...
            next: Some(format!(
                "Run: _harness/bin/harness-cli db rebuild --from {}",
    ...
    DoctorCheck {
        ...
        next: Some("Run: _harness/bin/harness-cli init".to_owned()),
    }
}
...
fn check_harness_cli_exists(config: &ResolvedConfig) -> DoctorCheck {
    ...
    match Command::new(&cli).arg("--version").output() {
        ...
        _ => DoctorCheck {
            ...
            next: Some("Build or install _harness/bin/harness-cli.".to_owned()),
        },
    }
}
...
fn check_harness_db_path_support(config: &ResolvedConfig) -> Result<DoctorCheck, DoctorError> {
    ...
    match output {
        ...
        _ => Ok(DoctorCheck {
            ...
            next: Some("Complete E04 US-028 or rebuild _harness/bin/harness-cli.".to_owned()),
        }),
    }
}
...
fn check_operation_log_support(config: &ResolvedConfig) -> Result<DoctorCheck, DoctorError> {
    ...
    match output {
        ...
        _ => Ok(DoctorCheck {
            ...
            next: Some("Complete E04 US-029 or rebuild _harness/bin/harness-cli.".to_owned()),
        }),
    }
}
...
fn harness_cli_path(config: &ResolvedConfig) -> std::path::PathBuf {
    config.repo_root.join("_harness/bin/harness-cli")
}

fn prepare_temp_schema(config: &ResolvedConfig, temp_root: &Path) -> Result<(), DoctorError> {
    let source = config.repo_root.join("_harness/scripts/schema");
    let target = temp_root.join("_harness/scripts/schema");
