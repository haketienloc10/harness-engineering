...
fn apply_changeset_path(
    ...
    let output = Command::new(config.repo_root.join("_harness/bin/harness-cli"))
...
mod tests {
    ...
    fn already_applied_changeset_marks_run_synced() {
        ...
        fs::create_dir_all(temp_dir.path().join("_harness/bin")).unwrap();
        fs::write(
...
"#,
        ...
        connection
            ...
            .unwrap();
        let cli_path = temp_dir.path().join("_harness/bin/harness-cli");
        fs::write(
            &cli_path,
