...
- `.codex/agents/`: canonical Codex agent files for Planner, Contract Reviewer, Generator, and Evaluator.
- `workflows/`: default and Epic lifecycle specifications for strict Codex project-scoped subagent orchestration.
...
3. Lifecycle Orchestrator: `run.yaml`, `run-manifest.md`, state transitions, gates, `next-role.sh`, Codex project-scoped subagent spawning, and `validate-run.sh`.

Harness core lifecycle execution requires real spawned subagents from named Codex project-scoped agents.
...
Harness lifecycle agents are installed in `.codex/agents/*.toml`. Coordinator must invoke `harness_planner`, `harness_contract_reviewer`, `harness_generator`, and `harness_evaluator`; if Codex subagent runtime is unavailable, block before Planner execution.
