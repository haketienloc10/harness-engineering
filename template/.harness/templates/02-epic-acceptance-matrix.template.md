# Epic Acceptance Matrix

## Epic-Level Acceptance Criteria

| ID | Acceptance Criteria | Required | Covered By Run(s) | Evidence | Status |
|---|---|---|---|---|---|
| AC-001 |  | Yes |  |  | planned |

## Behaviour Coverage

| Behaviour / Flow | Expected Result | Covered By | Verification Method | Status |
|---|---|---|---|---|
|  |  |  | Manual/E2E/API/Unit/Build | planned |

## Completion Rule

Epic chỉ được đóng khi:

- [ ] Tất cả required acceptance criteria đã Pass
- [ ] Các run liên quan đã có evaluator report
- [ ] Không còn blocker mở
- [ ] Decision log đã được cập nhật
- [ ] Final epic summary đã được viết hoặc overview đã được cập nhật
